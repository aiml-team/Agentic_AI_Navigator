...
# ───────────────────────────────────────────────────────────────
# Historical context:
#   Before Okta SSO, users were keyed on their corporate email
#   `<name>@bs.nttdata.com`. Okta now sends NameID as the bare
#   `<name>@nttdata.com` (no `bs.` subdomain). So the same person
#   appears under two different addresses.
#
# canonicalize_email() strips the `bs.` subdomain (and any other
# alias prefix we declare) so we can treat both forms as the
# same identity. The canonical form is the one WITHOUT prefixes,
# e.g.:
#     ashok.pottapalli@bs.nttdata.com  →  ashok.pottapalli@nttdata.com
#     Ashok.Pottapalli@nttdata.com     →  ashok.pottapalli@nttdata.com
#
# email_aliases() returns every lowercase variant of an email
# that should be treated as "the same user" — used to look up
# legacy rows in audit_log, NavigatorAdmins, etc.
# ═══════════════════════════════════════════════════════════════
...
def canonicalize_email(email: str) -> str:
    ...
    Strips known subdomain prefixes from the domain part so that
    e.g. `alice@bs.nttdata.com` and `alice@nttdata.com` both map
    to the same canonical key `alice@nttdata.com`.

    Only domains listed in _DOMAIN_ALIAS_RULES are affected;
    external emails are returned unchanged (just lowercased + trimmed).
    """
    if not email:
        return ""
...
def email_aliases(email: str) -> list[str]:
    ...
    with a known subdomain prefix re-attached.

    Used in SQL `WHERE … IN (?, ?, …)` clauses so historical rows
    under any alias form are matched. External (non-NTT) emails
    return just their lowercased canonical form.
    """
...
def identify_user(email: str) -> dict:
    ...
    aliases   = email_aliases(raw_email)                # e.g. ['x@bs.nttdata.com','x@nttdata.com']
    canonical = canonicalize_email(raw_email)           # e.g. 'x@nttdata.com'
...
def add_admin(email: str, name: str = "") -> dict:
    ...
    which domain alias Okta sends in the SAML NameID."""
    canonical = canonicalize_email(email)
